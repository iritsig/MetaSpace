...
def load_features(csv_path: str | Path) -> pd.DataFrame:
    """Load the feature table produced by MetaSpace."""
    p = Path(csv_path)
    if not p.exists():
...
def make_xy(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    """Select X (features) and y=true_match from a MetaSpace table."""
...
@click.option(
    ...
    help="Path to MetaSpace features CSV.",
)
@click.option(
    "--classifier",
    "classifiers",
...
def meta_learn(
    ...
    """Train/test a binary classifier to predict true_match from MetaSpace features."""
